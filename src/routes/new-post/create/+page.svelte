<script lang="ts">
	...
	const user = $derived<User>(data.user);
	...
	let currentActiveSize = $state(tabs[0]);
	let currentActiveTemplate = $state(templates[0]);

	$effect(() => {
		currentActiveTemplate = templates[0];
...
<main class="mt-24 md:mt-36 lg:mt-48 mb-36 md:mb-48">
	<div class="container">
		<div class="grid grid-cols-3 mb-8">
			...
			<div class="flex justify-center items-end gap-8 rounded-xl bg-stone-100 w-fit mx-auto px-8 justify-self-center">
				{#each tabs as tab}
					<button
						class="tab__button uppercase pb-2 pt-4 border-b-2 border-b-transparent transition duration-200 {currentActiveSize.pointer ===
						tab.pointer
							? 'tab--active'
							: ''}"
						onclick={() => {
							currentActiveSize = tab;
						}}
					>
						{tab.name}
					</button>
		...
		<div class="grid grid-cols-12 gap-24 items-start content-center">
			<div class="template__list grid grid-cols-2 gap-6 col-span-3" aria-labelledby="templateLabel">
				<div class="col-span-2">
					<div class="form-group text-area--2 xl:my-6">
				...
				{#each templates as template (template.id)}
					<button
						...
						currentActiveTemplate.documentID
			...
			<div class="col-span-9 flex justify-center items-start p-8 bg-stone-100 rounded-md">
				<div class="preview w-full h-[60dvh]" id="canvasPreviewWrapper">
					<Generator
						template={currentActiveTemplate}
						text={custom_text}
						size={currentActiveSize}
						sizes={tabs}
						{media_url}
					/>
				</div>
