...
>
	<div class="before-wrapper bg-black" bind:this={beforeOverlay}>
		{#await imageExists(config.before) then}
			<img src={config.before} alt="{config.alt} Before Image" class="opacity-80" />
		{:catch}
	...
	{#if tags.length > 0}
		<div class="tags__wrapper absolute bottom-2 inset-x-0 gap-2 flex items-center px-3 pointer-events-none z-50">
			{#each tags as tag}
				<div
