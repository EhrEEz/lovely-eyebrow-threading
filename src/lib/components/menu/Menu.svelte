        ...
        <div class="flex justify-stretch items-center w-full h-full h-100 py-8">
            <div class="flex flex-col flex-grow basis-0 gap-16">
                <div class="group nav__contacts" style="--delay:{0.4}s;">
                    <h3
                        class="mb-2 underline decoration-2 decoration-primary underline-offset-4"
                    >
                        Find us at
                    </h3>
                    <address class="not-italic">
                        {#if data.address.map_link}
                            <a
                    ...
                    {#if data.contact.phone}
                        <div>
                            <a
                                ...
                                class="text-2xl uppercase inline-flex items-center gap-4"
                            >
                                📞 {data.contact.phone}
                            </a>
                        </div>
                ...
                {#if data.contact.hours}
                    <div class="group nav__contacts" style="--delay:{0.45}s;">
                        <h3
                            class="mb-4 underline decoration-2 decoration-primary underline-offset-4"
                        >
                            Our Hours
                        </h3>
                        <ul class="flex flex-col gap-2">
                            <li class="uppercase mb-0">
                                <div class="text-lg">
                                    {data.contact.hours.weekdays.days}
                                </div>
                                <div class="text-2xl">
                                    {data.contact.hours.weekdays.timing}
                                </div>
                            </li>
                            <li class="uppercase mb-0">
                                <div class="text-lg">
                                    {data.contact.hours.weekends.days}
                                </div>
                                <div class="text-2xl">
                                    {data.contact.hours.weekends.timing}
                                </div>
                            </li>
                        </ul>
                    </div>
            ...
            <div class="flex-grow basis-0">
                <ul
                    class="flex flex-col justify-start items-start gap-4 flex-grow basis-0 ps-44"
                >
                    {#each links as link, index}
                        {#if link.slug !== data.cta}
                            <li
                                class="nav__item"
                                style="--delay:{((index + 1) * 0.08).toFixed(
                                    2,
                                )}s;--duration:{(
                            ...
                            >
                                <a
                                    href={link.href}
                                    class="text-5xl font-serif hover:underline underline-offset-8 decoration-primary"
                                >
                                    {link.name}
                                </a>
                            </li>
                ...
                <ul
                    class="flex flex-col justify-start items-start gap-2 mt-7 text-gray-700 flex-grow basis-0 ps-44"
                >
                    {#each secondary_links as link, index}
                        {#if link.slug !== data.cta}
                            <li
                                class="nav__item"
                                style="--delay:{(index + 5) *
                                    0.1}s;--duration:{(secondary_links.length -
                                    index +
                                    1) *
                                    0.2}s"
                            >
                                <a
                                    href={link.href}
                                    class="text-2xl uppercase hover:underline underline-offset-8 decoration-primary"
                                >
                                    {link.name}
                                </a>
                            </li>
...
<style>
    ...
    .nav__card.active .nav__item {
        ...
        animation: top-opacity-in var(--duration)
            cubic-bezier(0.25, 0.1, 0, 0.93) var(--delay) forwards;
    }
